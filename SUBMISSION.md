...
| T2b | ผ่าน | (เพิ่มพิเศษ) ปฏิเสธด้วยข้อความ `A ballot must rank exactly 3 candidates` ทดสอบกรณีส่งรายชื่อไม่ครบ 3 คน |
...
ทดสอบทั้งสองทาง: ผ่านเมนู CLI จริงในการรันครั้งเดียว และผ่าน `python -m unittest tests.test_scenarios` ซึ่งผ่าน 7 จาก 7 กรณี

## 4. ความแตกต่างระหว่างแบบที่ออกกับโปรแกรมจริง (ถ้ามี)
ระบุไม่เกิน 3 ข้อ
1. `Repository` และ `Tally` วาดไว้เป็นกล่องคลาสใน Class Diagram แต่ในโปรแกรมทำเป็นโมดูลระดับฟังก์ชัน เพราะทั้งสองไม่มีสถานะของตัวเอง การห่อเป็นคลาสจะเพิ่มชั้นโดยไม่ได้ประโยชน์
2. `ConsoleView.ask_ranking` เดิมออกแบบให้ถามอันดับทีละช่อง แต่เปลี่ยนเป็นรับทั้งสามรหัสในบรรทัดเดียว เพื่อให้บัตรที่ระบุไม่ครบ 3 อันดับถูกส่งเข้าไปให้ Model ตรวจและปฏิเสธพร้อมเหตุผลได้จริงตาม R2 แบบเดิมผู้ใช้จะยกเลิกไปก่อนเสมอ
